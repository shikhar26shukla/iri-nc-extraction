import {
  readCompanyKnowledgeBase,
  writeCompanyKnowledgeBase,
} from "@/lib/companies/store";
import {
  dbGetVersionEntries,
  dbInsertVersionSnapshot,
  dbListVersions,
} from "@/lib/db/queries/versions";
import type { NcSkillEntry, SkillBaseVersion } from "@/types";

function nextIndexedVersion(versions: SkillBaseVersion[]): number {
  return versions.length > 0 ? Math.max(...versions.map((v) => v.version)) + 1 : 1;
}

export async function listNcVersions(
  companyId: string
): Promise<SkillBaseVersion[]> {
  return dbListVersions(companyId, "nc");
}

export async function snapshotNcSkillBase(
  companyId: string,
  meta: Omit<SkillBaseVersion, "version" | "createdAt" | "entryCount">
): Promise<SkillBaseVersion | null> {
  const current = await readCompanyKnowledgeBase<NcSkillEntry>(companyId, "nc");
  if (current.length === 0) return null;

  const indexed = await dbListVersions(companyId, "nc");
  const nextVersion = nextIndexedVersion(indexed);
  const createdAt = new Date().toISOString();

  await dbInsertVersionSnapshot({
    companyId,
    skillType: "nc",
    version: nextVersion,
    source: meta.source,
    fileName: meta.fileName,
    mergeStats: meta.mergeStats,
    entryCount: current.length,
    entries: current,
    inIndex: true,
    createdAt,
  });

  return {
    version: nextVersion,
    createdAt,
    entryCount: current.length,
    source: meta.source,
    fileName: meta.fileName,
    mergeStats: meta.mergeStats,
  };
}

export async function restoreNcVersion(
  companyId: string,
  version: number
): Promise<{ restored: NcSkillEntry[]; snapshot: SkillBaseVersion | null }> {
  const restored = await dbGetVersionEntries<NcSkillEntry>(
    companyId,
    "nc",
    version
  );
  if (!restored) {
    throw new Error(`Version ${version} not found for company ${companyId}`);
  }

  const preRestoreSnapshot = await snapshotNcSkillBase(companyId, {
    source: "restore",
    fileName: `before-restore-v${version}`,
  });

  await writeCompanyKnowledgeBase(companyId, "nc", restored);

  return { restored, snapshot: preRestoreSnapshot };
}

export async function writeNcSkillBaseWithSnapshot(
  companyId: string,
  entries: NcSkillEntry[],
  meta: Omit<SkillBaseVersion, "version" | "createdAt" | "entryCount">
): Promise<SkillBaseVersion> {
  const current = await readCompanyKnowledgeBase<NcSkillEntry>(companyId, "nc");
  const indexed = await dbListVersions(companyId, "nc");
  const versionNum = nextIndexedVersion(indexed);
  const createdAt = new Date().toISOString();

  if (current.length > 0) {
    await dbInsertVersionSnapshot({
      companyId,
      skillType: "nc",
      version: versionNum,
      source: "upload",
      entryCount: current.length,
      entries: current,
      inIndex: false,
      createdAt,
    });
  }

  await writeCompanyKnowledgeBase(companyId, "nc", entries);

  const recordVersion = current.length > 0 ? versionNum + 1 : versionNum;
  await dbInsertVersionSnapshot({
    companyId,
    skillType: "nc",
    version: recordVersion,
    source: meta.source,
    fileName: meta.fileName,
    mergeStats: meta.mergeStats,
    entryCount: entries.length,
    entries,
    inIndex: true,
    createdAt,
  });

  return {
    version: recordVersion,
    createdAt,
    entryCount: entries.length,
    source: meta.source,
    fileName: meta.fileName,
    mergeStats: meta.mergeStats,
  };
}
