...
import type { NcSkillEntry, SkillBaseMergeStats, SkillBaseVersion } from "@/types";

const COMPANIES_ROOT = path.join(process.cwd(), "data", "companies");

function versionsDir(companyId: string): string {
  return path.join(COMPANIES_ROOT, companyId, "nc-versions");
}
...
export async function listNcVersions(
  companyId: string
...
export async function snapshotNcSkillBase(
  companyId: string,
  meta: Omit<SkillBaseVersion, "version" | "createdAt" | "entryCount">
): Promise<SkillBaseVersion | null> {
  const current = await readCompanyKnowledgeBase<NcSkillEntry>(companyId, "nc");
...
export async function restoreNcVersion(
  companyId: string,
  version: number
): Promise<{ restored: NcSkillEntry[]; snapshot: SkillBaseVersion | null }> {
  const snapshotPath = versionSnapshotPath(companyId, version);
  let restored: NcSkillEntry[];
  try {
    const raw = await fs.readFile(snapshotPath, "utf-8");
    restored = JSON.parse(raw) as NcSkillEntry[];
  } catch {
    throw new Error(`Version ${version} not found for company ${companyId}`);
  }

  const preRestoreSnapshot = await snapshotNcSkillBase(companyId, {
    source: "restore",
    fileName: `before-restore-v${version}`,
  });

  await writeCompanyKnowledgeBase(companyId, "nc", restored);

  return { restored, snapshot: preRestoreSnapshot };
}

export async function writeNcSkillBaseWithSnapshot(
  companyId: string,
  ...
  await writeCompanyKnowledgeBase(companyId, "nc", entries);
